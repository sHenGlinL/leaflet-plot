...
Draw.DoubleArrow = Draw.extend({
  ...
  onClick(evt) {
    ...
    if (
      lastTempNode &&
      evt.latlng.lat === lastTempNode._latlng.lat &&
      evt.latlng.lng == lastTempNode._latlng.lng
    ) {
      return;
    }
  ...
  onMousemove(evt) {
    if (this.tempGp.layerNodeLen >= 2) {
      ...
      const index =
        this.tempGp.layerNodeLen === 2 ? 2 : this.tempGp.layerNodeLen - 1;
      this.tempGp.layerNode[index] = [evt.latlng.lat, evt.latlng.lng];
